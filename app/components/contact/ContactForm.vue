...
function validateName(value: string): boolean {
  ...
  if (!NAME_REGEX.test(value)) {
    nameError.value = 'El nombre solo debe contener letras y espacios (2-50 caracteres)'
    return false
  }
  nameError.value = ''
  return true
...
function validateLastname(value: string): boolean {
  ...
  if (!NAME_REGEX.test(value)) {
    lastnameError.value = 'El apellido solo debe contener letras y espacios (2-50 caracteres)'
    return false
  }
  lastnameError.value = ''
  return true
...
function validateMessage(value: string): boolean {
  ...
  if (!MESSAGE_REGEX.test(value)) {
    messageError.value = 'El mensaje solo puede contener caracteres regulares (máx 256 caracteres)'
    return false
  }
  messageError.value = ''
  return true
...
async function handleSubmit() {
  ...
  try {
    ...
    if (response.ok) {
      successMessage.value = '¡Formulario enviado con éxito! Gracias por contactarnos.'
    ...
    clientError.value = 'Hubo un problema al conectar con el servidor. Por favor, inténtalo más tarde.'
  } finally {
    sending.value = false
  }
}
...
<template>
  ...
  >
    <div class="h-[94vh] bg-color-1 p-8 pt-24 md:p-12 md:pt-[10.5rem] md:pr-16 lg:pl-16">
      ...
      >
        ...
        <div class="flex flex-col gap-4 lg:flex-row">
          <div class="w-full">
            <input
              ...
              @input="onNameInput"
            />
            <p v-if="nameError" id="name-error-msg" class="mt-1 text-sm text-[#ef4444]">
              {{ nameError }}
            </p>
          </div>
          <div class="w-full">
            ...
            />
            <p v-if="lastnameError" id="lastname-error-msg" class="mt-1 text-sm text-[#ef4444]">
              {{ lastnameError }}
            </p>
          </div>
        </div>
        <div class="flex flex-col gap-4 lg:flex-row">
          <div class="w-full">
            <input
              ...
              @input="onEmailInput"
            />
            <p v-if="emailError" id="email-error-msg" class="mt-1 text-sm text-[#ef4444]">
              {{ emailError }}
            </p>
          </div>
          <div class="w-full">
            <input
              ...
              @input="onPhoneInput"
            />
            <p v-if="phoneError" id="phone-error-msg" class="mt-1 text-sm text-[#ef4444]">
              {{ phoneError }}
            </p>
          </div>
        </div>
        ...
        <p v-if="messageError" id="message-error-msg" class="mt-1 text-sm text-[#ef4444]">
          {{ messageError }}
        </p>

        <p v-if="clientError" class="rounded border border-[#f87171] bg-[#fecaca] py-1 pl-2 text-[#ef4444]">
          {{ clientError }}
        </p>

        <p v-if="successMessage" class="rounded border border-[#22c55e] bg-[#bbf7d0] py-1 pl-2 text-[#15803d]">
          {{ successMessage }}
        </p>
